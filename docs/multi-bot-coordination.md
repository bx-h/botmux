...
截至当前验证，`available_bots` 主问题在 upstream 已有对应修复，本 fork 只需要在 rebase 后保留回归覆盖。后续重点是第二类：**bot-to-bot 多次重复 @ 的通用治理**。

这不是 `/role`、团队角色、能力标签或 `botmux dispatch/report` 的替代方案：

- `/role` / 默认角色 / 能力标签解决“每个 bot 是谁、擅长什么”。
- `botmux dispatch/report` 解决“主 bot 结构化开子话题派活、子 bot 回报到正确上下文”。
- 本方案只处理普通自然语言 bot-to-bot `@` 中的幂等与重复唤起问题，尤其是同一协作实例里多个上游消息连续点名同一个目标 bot。

如果协作入口已经能稳定使用 `botmux dispatch --into`，应优先走 upstream 原生路径；ledger 只作为自然语言 `@` 的防抖/幂等补层。
...
- 已移除业务特定任务 key，例如 `intro`。
- 已生成通用 `taskKey`：`coordinationId + assignee + objective + expectedOutput`。
- 已单独生成 `inputRevisionHash`：约束、引用摘要、附件和返工信号。
- 已为 bot-to-bot mention 注入 `coordination_context`。
- 当前实现仅支持最小 `open/skipped` 状态和精确重复 bot mention 跳过；完整 `queued/running/completed/failed/superseded` 状态还未落地。

### P2：短窗口合并

- 待实现 per-target fan-in buffer。
- 当前已有同一 `coordinationId + assignee + taskKey` 下的精确重复抑制和 source message 归档；还没有完整 5-15 秒窗口合并。
- 继续补 `queued/running/completed` 的可观测日志、低频反馈和执行状态闭环。
...
- `available_bots` 主问题在 upstream 已有修复，本地后续只在 rebase 后保留回归测试。
- 当前源码已移除 `intro` / `isIntroLike` 特判，使用通用 `taskKey + inputRevisionHash`。
- 当前已有最小 `coordination-ledger` 和 `coordination_context` prompt 注入；它只应视为自然语言 `@` 去重 MVP，剩余重点是 fan-in buffer、状态闭环、与 upstream `dispatch/report` 边界对齐，以及 handoff contract 产品化。
...
3. 第一优先级是复核通用 `taskKey + inputRevisionHash` 在自然语言 handoff、附件、引用消息下的稳定性，避免误合并或漏合并。
4. 第二优先级是把状态分层继续压实：输入分类、ledger 持久状态、prompt 展示状态不要混用。
5. 第三优先级是补最小 fan-in 或 reservation 语义；当前已有精确重复抑制，但还不能宣称完整 5-15 秒窗口合并。
...
本文档路径：`docs/multi-bot-coordination.md`
