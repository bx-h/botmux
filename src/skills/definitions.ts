...
## 物理事实（先记牢）
- **你和子 bot 之间没有直连**，只能靠飞书消息触发；**没有请求-响应关联**——子 bot 干完用 \`botmux report\` 把回报贴回**它自己的子项目话题**并 @ 你。普通群 chat-scope 下，这个 @ 会折回你的群级会话，但可见讨论仍留在该任务话题；你被唤起后去读任务板和该话题拿结构化状态。
...
- **反方向同理（你 → 子 bot）**：子 bot 的会话只在它的子话题里。你要跟某个子 bot 说话（追问/补任务/确认），**一律 \`botmux dispatch --into <子话题root> --bot <子bot>\`** 发进它的子话题——**绝不要在主群 \`botmux send --mention <子bot>\`**，那条 @ 到不了它在子话题的会话，反而会在主群另起一个无上下文的新会话。子 bot 回报后你通常只需聚合，不必回它；要回也走 \`--into\`。（\`botmux send\` 已加护栏：误 @ 活跃子 bot 会被拦下并提示，确需强发才 \`--anyway\`。）
...
- 然后用 \`botmux report "子项目X 完成 + 产出位置"\` 回报（它会贴回本任务话题并 @ 主 bot；不要绕过 report 另发群聊顶层消息。\`botmux dispatch\` 已把这条「完成回报」协议自动追加进简报，子 bot 照做即可）；
...
### 6. 收结果 + 推进
子 bot \`botmux report\` → 回报留在对应子项目话题并唤起你 → 读任务板和该话题确认完成、看产出。然后：
