...
describe('bot-registry grant additions', () => {
  ...
  it('parses regularGroupReplyMode: keeps chat|new-topic|shared, drops invalid/absent to undefined', () => {
    expect(parseBotConfigsFromText(JSON.stringify([{ larkAppId: 'rg0', larkAppSecret: 's', regularGroupReplyMode: 'chat' }]))[0].regularGroupReplyMode).toBe('chat');
    ...
    // Missing now means the shared/topic default; 'chat' is an explicit opt-out.
