...
| `soulPath` | Optional persona file injected into new sessions and reforks as a low-priority `<bot_persona>` block. Relative paths are resolved from the `bots.json` directory and must stay inside its `souls/` subtree; absolute paths are checked against the same boundary. It cannot override system, safety, tool, approval, sandbox, or current-user instructions |
